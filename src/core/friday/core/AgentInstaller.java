...
public class AgentInstaller {
	...
	public static void attach() {
		...
		for(String pid : iString.split("[ ]")){
			...
			try{	attach(pid);System.out.println("[Done]");	}catch(Throwable ignored){
	...
	private static void attach(String pid) throws AttachNotSupportedException, IOException, AgentLoadException, AgentInitializationException{
		
			
		VirtualMachine vm = VirtualMachine.attach(pid);
		
		vm.loadAgent(
				new File(InitConstants.CORE_JAR_NAME).getAbsolutePath(), 
				null
				);
